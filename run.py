...
web_search_tool_dict = {"type": "web_search", "name": "web_search"}  # Gemini requires valid function names
...
def initialize_llm() -> bool:
    ...
    google_ready = bool(google_api_key)
    if google_ready and (llm is None or getattr(llm, 'google_api_key', None) != google_api_key):
        try:
            base_llm = ChatGoogleGenerativeAI(model="gemini-1.5-pro", google_api_key=google_api_key, temperature=0.2)
            llm = base_llm; llm_with_search = base_llm.bind_tools(tools_list_for_binding)
            print(f"LLM initialized (gemini-1.5-pro). Bound: {web_search_tool_dict}")
            return True
...
with st.sidebar: # Sidebar UI (No Change)
    st.header("🔑 Google API Key"); google_api_key_input = st.text_input("API Key", type="password", value=os.environ.get("GOOGLE_API_KEY", ""), help="Needed for LLM nodes.", key="api_key_input_sidebar"); api_key_in_env = os.environ.get("GOOGLE_API_KEY"); api_key_changed = (google_api_key_input != api_key_in_env if api_key_in_env else bool(google_api_key_input))
    if api_key_changed:
        if google_api_key_input: os.environ["GOOGLE_API_KEY"] = google_api_key_input
        elif "GOOGLE_API_KEY" in os.environ: del os.environ["GOOGLE_API_KEY"]
        llm = None; llm_with_search = None; llm_ready = initialize_llm(); st.session_state.google_api_key_provided = bool(google_api_key_input)
        if api_key_changed: st.session_state.compiled_graph = None; st.rerun()
    else: llm_ready = initialize_llm(); st.session_state.google_api_key_provided = bool(os.environ.get("GOOGLE_API_KEY"))
    if llm_ready: st.success("LLM Initialized.", icon="✅")
    elif st.session_state.get("google_api_key_provided"): st.error("LLM Init Failed.", icon="🔥")
